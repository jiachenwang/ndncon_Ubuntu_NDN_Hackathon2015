...
SegmentFetcher::fetchNextSegment
  (const Interest& originalInterest, const Name& dataName, boost::uint64_t segment)
{
   ...
   const ptr_lib::shared_ptr<Data>& data)
{
  ...
  if (!endsWithSegmentNumber(data->getName()))
    ...
    boost::uint64_t expectedSegmentNumber = contentParts_.size();
    if (currentSegment != expectedSegmentNumber)
      ...
      if (data->getMetaInfo().getFinalBlockId().getValue().size() > 0) {
        boost::uint64_t finalSegmentNumber;
        try {
